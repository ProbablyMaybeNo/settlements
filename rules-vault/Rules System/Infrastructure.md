...
- **Triggers can't be destroyed.** A terminal or a feature's controls are permanent fixtures — they can't be shot off the board. You deny an *enemy's* control only by **FREEZING** their terminal with an interrupt ([[Hacking#Interrupt]]) — and **manual operation in base contact always remains** available to anyone who reaches it.
...
- **Interrupt:** use [[Skills#INT]] and [[Keywords#FREEZE]].
...
- **Power Generator / Junction** — *Utilities · enabler*. The optional macro-toggle: while off, all infrastructure in its zone is **Powered Down**. Operating the Generator (hack or manual) powers the zone on or off. It can't be destroyed — deny it by **FREEZING** its terminal (an interrupt).
