...
### Tier 1 — Good
- **Hacker** — As an Action beside a Hackable device, pass an INT test, or an opposed INT test if defended, to access data, loop a camera, silence an alarm or toggle an unlocked Powered system. *(Hacker · Objective specialist)*
...
### Tier 2 — Great
- **Computer Whiz** — As an Action at an accessed terminal, hack one **Linked** device on its local network. Resolve defended systems with an opposed INT test. *(Hacker · Remote control)*
...
- **Counter-Hack** — When Readied at a device or terminal, react to a hostile hack with an opposed INT test. On a win, cancel it and make the hostile system Compromised. *(Hacker nerfer · Tech support)*
...
### Tier 3 — Amazing
- **Mastermind** — As an Action at an accessed terminal, make one INT test to operate two different Linked non-weapon device functions. A defended network opposes once; failure operates neither. *(Hacker · Controller)*
