...
- [x] **~~Do surplus whiffed dice Pin?~~ CLOSED 2026-08-29 — and wider than asked.** Ross ruled **all outcomes count**: every die that hits converts to **+1 Stress**, whether it passed the Injury roll or failed it. Wounds still never stack. Measured — at WND 2–3 P(Break test) goes **14.5% → 28.5%** with E[wounds] untouched at 0.738; at WND 1 E[Stress] goes 0.720 → **1.062**. **Costs nothing at the till** (AD 2/3 price at 38/63 either way — the catalogue prices wounds and this adds none) and is emphatically not free at the table. Propagated to §§9/11/15, Damage, Weapons, Morale and three ledger cards.
...
- [x] **~~Then, and only then, surplus Attack Dice passes → Stress~~ ADOPTED 2026-08-29 with persistence.** The plan was to test persistence alone first; Ross ruled both the same day, and they interlock — surplus-to-Stress does **nothing at WND 1** without persistence, so splitting them would have left the second rule untestable. **The table now tests the stacked case**: a downed model's Stress roughly **2.2×** (0.66 → 1.44), wasted revivals ~1-in-11 → nearly **1-in-4**. Valve still in reserve — *"a Stabilised fighter returns Shaken, however much Stress it had."*
