...
SHEETS = [
    ...
    ("Skills", "crew",
     [("Skill", 24), ("Stat", 8), ("Tier", 7), ("Requires", 20), ("Effect", 70),
      ("Role", 22), ("Notes", 26)],
     ["Squeeze", "STR", "T2", "Grapple",
      "As an Action, make an opposed STR test against an enemy you have Grappled. On a win, it suffers an automatic unarmed hit; make only the Injury roll.",
      "Grappler", "Cannot be used without Grapple"]),
...
LEGEND = [
    ...
    ("", ""),
    ("The Requires column (Skills)", ""),
    ("Name the skill it needs", "A skill that uses a state only another skill can create must name it. Squeeze needs Grapple; without it the card is dead."),
    ("Climbing a chain costs slots", "To take a T3 in a chain you need its T2, which needs its T1 — so a Leader spending all three slots on one chain is the whole build. That is the intended cost."),
    ("Leave blank if standalone", "Most skills need nothing. Only the chained ones fill this in."),
    ("", ""),
    ("The Ruled column", ""),
